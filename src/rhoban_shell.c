...
static void displayHelp(char parameter) {
  char buffer[256];
  unsigned int i;
 ...
 */
SHELL_COMMAND(help, "Displays the help about commands") { displayHelp(0); }
...
SHELL_COMMAND(echo, "Switch echo mode. Usage echo [on|off]") {
  if ((argc == 1 && strcmp("on", argv[0])) || shell_echo_mode == 0) {
    shell_echo_mode = 1;
    printf("Echo enabled\n");
  } else {
    shell_echo_mode = 0;
 ...
 */
char shell_execute(char *command_name, unsigned int command_name_length,
 ...
 * Stops the shell
 */
void shell_enable() { shell_last_ok = 0; }

static char has_available_bytes() {
